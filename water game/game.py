...
counter = 0
bg = pg.image.load('sky.jpg')
class Player(pg.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pg.image.load('umbrella.png')
...
class Object(pg.sprite.Sprite):
    def __init__(self,x,y):
        super().__init__()
        self.image = pg.image.load('waterdrop3.png').convert_alpha()
        self.image = pg.transform.scale(self.image,(20,35))
        self.rect = self.image.get_rect()
        self.mask = pg.mask.from_surface(self.image)
        self.rect.x = x
        self.rect.y = y
        self.speed_falling = 5
    def destroyed(self):      
        pass
    def update(self):
        self.destroyed()
...
class Bullet(pg.sprite.Sprite):
    def __init__(self,player_x,player_y):
        ...
        self.speed = 30
        self.sound = pg.mixer.Sound('explosion.wav')
...
while running:
    ...
    #spawn objects
    if random.randint(15,30) == 15:
        object_sprite = Object(random.randrange(screen_w),0)
        if pg.sprite.spritecollideany(object_sprite,object_group) == None :
            object_group.add(object_sprite)

    #collision of obj and player
    if pg.sprite.spritecollideany(player, object_group,pg.sprite.collide_mask):
        pg.quit()
        exit()       
